...
export default function SignupPage() {
  ...
  async function onSignup(e: React.SubmitEvent<HTMLFormElement>) {
    e.preventDefault();
    setError(null);
    setIsLoading(true);
    ...
    <main className="min-h-screen flex items-center justify-center p-6 bg-[#111714]">
      <div className="w-full max-w-sm space-y-4 bg-white p-6 rounded-lg shadow-lg">
        ...
        <form className="flex flex-col gap-3" onSubmit={onSignup}>
          ...
          <button
            type="submit"
            className="w-full rounded bg-black text-white py-2 cursor-pointer hover:bg-gray-800 transition disabled:bg-gray-400 flex items-center justify-center"
            disabled={isLoading}
          >
            {isLoading ? <Loader width="1.5rem" height="1.5rem" fill="white" /> : 'Registrarse'}
          </button>
        </form>
